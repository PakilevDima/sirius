...
def sum_text(SENTENCES_COUNT: int, file_path: str) -> str:
    ...
    Функция реализует сжатие текстом методом LSA с помощью библиотеки sumy, которая предоставляет большой функционал для
    ...
    parser = PlaintextParser.from_string(file_path, Tokenizer(LANGUAGE))
